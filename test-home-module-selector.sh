...
HOME_MANAGER_DIR="${HOME_MANAGER_DIR:-$HOME/.home-manager}"
HOME_MODULES_PATH="${HOME_MODULES_PATH:-$HOME_MANAGER_DIR/modules.nix}"
...
usage() {
  cat <<'EOF'
Usage: test-home-module-selector.sh [options]

Scans Home Manager module options, presents a TUI checklist,
then prints the modules.nix it would generate.

Options:
  --home-manager-dir PATH   Home manager root (default: ~/.home-manager)
  --home-modules PATH       Home modules output path label (default: ~/.home-manager/modules.nix)
...
discover_source_defaults() {
  ...
  done < <(find "$root_dir" -type f -name '*.nix' ! -name 'modules.nix' ! -name 'local.nix' -print0)
}
...
main() {
  ...
  mapfile -t home_options < <(discover_enable_options "$HOME_MANAGER_DIR")
  discover_source_defaults "$HOME_MANAGER_DIR"

  if [[ ${#home_options[@]} -eq 0 ]]; then
    echo "Warning: no home enable options discovered in $HOME_MANAGER_DIR" >&2
  fi

  selected="$(pick_modules "Home Manager Modules" "Select home modules to enable" "${home_options[@]}")"
